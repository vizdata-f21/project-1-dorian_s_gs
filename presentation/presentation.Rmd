...
superbowl_animals <- superbowl %>%
  ...
  summarise(total_view_count = sum(view_count, na.rm = TRUE),
            count = n(),
            avg_view_count = total_view_count/count) %>%
  rename(variable = animals)
```
...
```{r}
  select(animals, celebrity, use_sex) %>%
  pivot_longer(cols = c(animals, celebrity, use_sex), names_to = "variable", values_to = "count")

```

```{r Q1-plot-1}
superbowl %>%
  ggplot(aes(x = animals, celebrity, use_sex, y = view_count, fill = animals)) +
...
superbowlQ1P2 <- superbowlQ1P2[!duplicated(superbowlQ1P2), ]

superbowl_plot <- superbowlQ1P2 %>%
...
superbowl_plot_long <- superbowl_plot_long %>%
  na.omit() %>%
  filter(value != 0)
view(superbowl_plot_long)


ggplot(superbowl_plot_long, aes(year, value, colour = variable)) + 
  geom_point() 
