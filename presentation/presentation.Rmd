...
superbowl %>%
  mutate(view_category = paste(view_category, "Views")) %>%
   mutate(view_category = fct_relevel(
    view_category, "Few Views","Some Views","Medium Views","Many Views", "High Views", "Viral Views" 
  )) %>%
  facet_order = factor(view_category, levels=c("Few Views","Some Views","Medium Views","Many Views", "High Views", "Viral Views")) %>%
ggplot(aes(x =view_count, y=interactions)) +
  geom_area(aes(fill = view_category, alpha=0.5), show.legend = NULL)+
  geom_line() +
  facet_wrap(vars(facet_order),  
             scales ="free",
             nrow = 3,
             strip.position = "top") +
